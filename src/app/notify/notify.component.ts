...
constructor(
  ...
  loadMarkerData(name_id: string) {
    ...
    const apiUrl = `https://just-scan-me-backend.onrender.com/api/marker/${encodeURIComponent(name_id)}`;
  ...
  notifyContent(): void {
    ...
    const now = new Date();
    const report_time = `${now.getDate().toString().padStart(2, '0')}-${(now.getMonth() + 1).toString().padStart(2, '0')}-${now.getFullYear()} ${now.getHours().toString().padStart(2, '0')}:${now.getMinutes().toString().padStart(2, '0')}`;
    ...
    const apiUrl = `https://just-scan-me-backend.onrender.com/api/save-electric-pole`;
